...
Location: New York City (on-site)`,
  ...
  const handleScrapedJobCreated = async (scrapedJob: JobDescription) => {
    try {
      // Stamp a sequential ID if the scraper didn't assign one
      const jobWithSeqId: JobDescription = {
        ...scrapedJob,
        sequentialId: scrapedJob.sequentialId || getNextSequentialId()
      };

      // Save the scraped job (also overwrites the copy JobPreviewModal already saved without a seqId)
      await saveJobDescription(jobWithSeqId);

      // Update state
      setState(prev => ({
        ...prev,
        jobDescriptions: [...prev.jobDescriptions, jobWithSeqId]
      }));

      // Log activity
      logActivity(jobWithSeqId, 'field_updated', {
        field: 'imported',
        toValue: true,
